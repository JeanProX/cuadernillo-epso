...
const actividades = [
  {
    ...
    instruccion:
      "Reflexiona sobre tu situación actual y la forma en que te percibes.",
    ...
    campos: [
      ...
      {
        id: "queMeDescribe",
        etiqueta: "¿Qué me describe?",
        tipo: "textarea"
      },
      {
        id: "comoMePercibo",
        etiqueta: "¿Cómo me percibo?",
        tipo: "textarea"
      }
    ]
  },
  {
    ...
    instruccion:
      "Identifica factores relacionados con tu salud física, mental y social.",
  ...
  },
  {
    ...
    instruccion:
      "Registra tus datos y los de un familiar. El IMC se calcula automáticamente.",
    ...
    campos: [
      ...
      {
        id: "pesoFamiliar",
        etiqueta: "Peso de un familiar (kg)",
        tipo: "number",
        step: "0.1"
      },
      {
        id: "estaturaFamiliar",
        etiqueta: "Estatura del familiar (m)",
        tipo: "number",
        step: "0.01"
      },
      {
        id: "imcFamiliar",
        etiqueta: "IMC del familiar",
        tipo: "resultado"
      },
      {
        id: "factoresPeso",
        etiqueta:
          "Aspectos emocionales o circunstanciales relacionados con el peso",
        tipo: "textarea",
        ancho: true
      }
    ]
  },
  {
    ...
    instruccion:
      "Escribe hasta siete excesos e indica el área a la que pertenecen.",
    ...
    campos: Array.from({ length: 7 }, (_, i) => ({
      ...
      tipo: "text",
      ayuda:
        "Ejemplo: alimentación, emocional, laboral o social"
    }))
  },
  {
    titulo: "Jarra y plato: hoy y mañana",
    instruccion:
      "Describe cómo comes y bebes hoy, y qué modificaciones te gustaría realizar mañana.",
    imagen: "img/page-07.png",
    campos: [
      {
        id: "aguaHoy",
        etiqueta: "Jarra de hoy: cantidad de agua",
        tipo: "textarea"
      },
      {
        id: "platoHoy",
        etiqueta: "Plato de hoy: alimentos y distribución",
        tipo: "textarea"
      },
      {
        id: "aguaManana",
        etiqueta: "Jarra de mañana: cambios deseados",
        tipo: "textarea"
      },
      {
        id: "platoManana",
        etiqueta: "Plato de mañana: cambios deseados",
        tipo: "textarea"
      }
    ]
  },
  {
    ...
    instruccion:
      "Registra lo que compras habitualmente y posibles sustituciones.",
    imagen: "img/page-08.png",
    campos: [
      {
        id: "listaSuper",
        etiqueta: "Mi lista del súper",
        tipo: "textarea"
      },
      {
        id: "mejorOpcion",
        etiqueta: "¿Es la mejor opción o es necesario?",
        tipo: "textarea"
      },
      {
        id: "cambiarPor",
        etiqueta: "Podría cambiarlo por...",
        tipo: "textarea",
        ancho: true
      }
    ]
  },
  {
    ...
    instruccion:
      "Escribe lo positivo, negativo e interesante de los factores relacionados con tu salud.",
  ...
  },
  {
    ...
    instruccion:
      "Registra horarios posibles para hacer pausas activas.",
    imagen: "img/page-10.png",
    campos: [
      {
        id: "pausaCasa",
        etiqueta: "Horario en casa",
        tipo: "time"
      },
      {
        id: "pausaTrabajo",
        etiqueta: "Horario en el trabajo",
        tipo: "time"
      },
      {
        id: "pausaOtro",
        etiqueta: "Horario en otro sitio",
        tipo: "time"
      },
      {
        id: "actividadPausa",
        etiqueta: "¿Qué actividad realizarías?",
        tipo: "textarea",
        ancho: true
      }
    ]
  },
  {
    ...
    instruccion:
      "Enlista acciones y roles personales, familiares y laborales.",
  ...
  },
  {
    ...
    instruccion:
      "Escribe frases motivadoras, aprendizajes y compromisos personales.",
    imagen: "img/page-12.png",
    campos: [
      {
        id: "fraseMotivadora",
        etiqueta: "Frase motivadora",
        tipo: "textarea"
      },
      {
        id: "aprendizaje",
        etiqueta: "¿Qué aprendí?",
        tipo: "textarea"
      },
      {
        id: "objetivo",
        etiqueta: "Mi objetivo",
        tipo: "textarea"
      },
      {
        id: "compromiso",
        etiqueta: "Mi compromiso",
        tipo: "textarea"
      }
    ]
  }
...
function iniciar(continuar = false) {
  ...
  actual =
    continuar && Number.isInteger(datos.ultimaActividad)
      ? Math.min(
...
function mostrarActividad(indice) {
  actual = Math.max(
    0,
    Math.min(indice, actividades.length - 1)
  );
  ...
  $("textoActividad").textContent =
    `Actividad ${actual + 1} de ${actividades.length}`;

  const porcentaje = Math.round(
    ((actual + 1) / actividades.length) * 100
  );
  ...
  $("btnAnterior").textContent =
    actual === 0 ? "← Inicio" : "← Anterior";

  $("btnSiguiente").textContent =
    actual === actividades.length - 1
      ? "Finalizar ✓"
      : "Guardar y continuar →";
...
function renderCampos(act) {
  ...
  $("formularioActividad").innerHTML = act.campos
    .map((campo) => {
      const clase = campo.ancho
        ? "grupo-campo ancho"
        : "grupo-campo";
...
function guardarActividad(notificar = true) {
  ...
  if (notificar) {
    mostrarNotificacion("Respuestas guardadas");
  }
}
...
function calcularIMC() {
  ...
  const estaturaFamiliar = Number(
    $("estaturaFamiliar")?.value
  );
  ...
  datos.imcFamiliar =
    pesoFamiliar > 0 && estaturaFamiliar > 0
      ? (
          pesoFamiliar /
          (estaturaFamiliar * estaturaFamiliar)
        ).toFixed(2)
      : "";
  ...
  if ($("imc")) {
    $("imc").textContent =
      datos.imc || "Sin calcular";
  }

  if ($("imcFamiliar")) {
    $("imcFamiliar").textContent =
      datos.imcFamiliar || "Sin calcular";
  }
}

function validarActividadActual() {
  const actividad = actividades[actual];

  const camposRespondibles = actividad.campos.filter(
    (campo) => campo.tipo !== "resultado"
  );

  let existeRespuesta = false;
  let primerCampoVacio = null;

  camposRespondibles.forEach((campo) => {
    const elemento = $(campo.id);
  ...
  if (!existeRespuesta) {
    mostrarNotificacion(
      "Escribe al menos una respuesta para continuar"
    );
...
function finalizar() {
  ...
  $("resumenFinal").innerHTML = `
    ...
    ${actividades.length} de ${actividades.length}
  `;
...
function abrirResumenLectura() {
  ...
  $("datosResumen").innerHTML = `
    <strong>Participante:</strong>
    ${esc(datos.nombre || "Sin nombre")}
    <br>

    <strong>Fecha:</strong>
    ${esc(datos.fecha || "Sin fecha")}
  `;

  $("contenidoResumenLectura").innerHTML =
    actividades
      .map((actividad, indice) => {
        const preguntas = actividad.campos
          .map((campo) => {
...
document.addEventListener(
  ...
  () => {
    ...
    $("btnAmpliar").addEventListener(
      "click",
      abrirImagen
    );

    $("imagenActividad").addEventListener(
      "click",
      abrirImagen
    );
    ...
    $("btnReflexiones").addEventListener(
      "click",
      abrirResumenLectura
    );

    $("btnCerrarResumen").addEventListener(
      "click",
      cerrarResumenLectura
    );
    ...
    $("btnImprimirResumen").addEventListener(
      "click",
      () => window.print()
    );
